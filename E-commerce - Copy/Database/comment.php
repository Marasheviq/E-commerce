...
$db_user = "root";
$db_pass = "";
$db_name = "Ecommerce"; // Corrected database name
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Uncomment to debug
    // var_dump($comment, $CustomerId, $ProductId);
    
    // Check if the customer exists and has made a purchase
    $customer_check_sql = "
    SELECT c.CustomerId 
    FROM Customer c
    ...
    if (mysqli_stmt_num_rows($stmt) > 0) {
        // Customer exists and has made a purchase, proceed with comment submission
        if ($comment && $ProductId) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                // Capture the error from the query execution
